...
myKeys conf@XConfig {XMonad.modMask = modm} =
  M.fromList $
    ...
    [ ((mod1Mask, xK_Return), spawn $ XMonad.terminal conf),
      ...
      ((mod1Mask, xK_p), rofiLauncher),
      ((mod1Mask, xK_o), centerlaunch),
      ((mod1Mask .|. shiftMask, xK_o), ewwclose),
      -- launch eww sidebar
      ((mod1Mask, xK_s), sidebarlaunch),
      ((mod1Mask .|. shiftMask, xK_s), ewwclose),
      -- Audio keys
      ...
      -- Screenshot
      ((0, xK_Print), maimcopy),
      ((mod1Mask, xK_Print), maimsave),
      -- My Stuff
      ((mod1Mask, xK_b), spawn "exec ~/bin/bartoggle"),
      ((mod1Mask, xK_z), spawn "exec ~/bin/inhibit_activate"),
      ((mod1Mask .|. shiftMask, xK_z), spawn "exec ~/bin/inhibit_deactivate"),
      ((mod1Mask .|. shiftMask, xK_a), clipboardy),
      -- Turn do not disturb on and off
      ((mod1Mask, xK_d), spawn "exec ~/bin/do_not_disturb.sh"),
      -- close focused window
      ((mod1Mask, xK_q), kill),
      -- GAPS!!!
      ((mod1Mask .|. controlMask, xK_g), sendMessage ToggleGaps), -- toggle all gaps
      ((mod1Mask .|. shiftMask, xK_g), sendMessage $ setGaps [(L, 30), (R, 30), (U, 30), (D, 45)]), -- reset the GapSpec
      ((mod1Mask .|. controlMask, xK_t), sendMessage $ IncGap 10 L), -- increment the left-hand gap
      ((mod1Mask .|. shiftMask, xK_t), sendMessage $ DecGap 10 L), -- decrement the left-hand gap
      ((mod1Mask .|. controlMask, xK_y), sendMessage $ IncGap 10 U), -- increment the top gap
      ((mod1Mask .|. shiftMask, xK_y), sendMessage $ DecGap 10 U), -- decrement the top gap
      ((mod1Mask .|. controlMask, xK_u), sendMessage $ IncGap 10 D), -- increment the bottom gap
      ((mod1Mask .|. shiftMask, xK_u), sendMessage $ DecGap 10 D), -- decrement the bottom gap
      ((mod1Mask .|. controlMask, xK_i), sendMessage $ IncGap 10 R), -- increment the right-hand gap
      ((mod1Mask .|. shiftMask, xK_i), sendMessage $ DecGap 10 R), -- decrement the right-hand gap

      -- Rotate through the available layout algorithms
      ((modm, xK_space), sendMessage NextLayout),
      --  Reset the layouts on the current workspace to default
      ((modm .|. shiftMask, xK_space), setLayout $ XMonad.layoutHook conf),
      ((mod1Mask, xK_space), scratchpadSpawnActionCustom "kitty -T=scratchpad --class=scratchpad"),
      -- , ((modm .|. controlMask , xK_space), scratchpadSpawnActionCustom "st -w '' -t 'scratchpad' -c 'scratchpad'")
      ...
      -- Quit xmonad
      ((mod1Mask .|. shiftMask, xK_q), spawn "~/bin/powermenu.sh"),
      -- Restart xmonad
      ((modmodm, xK_q), spawn "xmonad --recompile; xmonad --restart"),
      -- Run xmessage with a summary of the default keybindings (useful for beginners)
      ((mod1Mask .|. shiftMask, xK_h), spawn ("echo \"" ++ help ++ "\" | xmessage -file -"))
    ]
      ++
      --
  ...
  fullscreenManageHook <+> manageDocks
    <+> composeAll
      [ className =? "MPlayer" --> doFloat,
        ...
        className =? "electrum" --> doFloat,
        className =? "Tor Browser" --> doFloat,
        className =? "Xmessage" --> doFloat,
        className =? "scratchpad" --> doF (W.swapMaster >> W.swapMaster) <+> doFloat,
        className =? "feh" --> doFloat,
        className =? "mpv" --> doF W.swapMaster <+> doFloat,
...
defaults =
  def
    { -- simple stuff
      ...
      layoutHook = gaps [(L, 30), (R, 30), (U, 30), (D, 45)] $ spacingRaw True (Border 10 10 10 10) True (Border 10 10 10 10) True $ smartBorders myLayout,
